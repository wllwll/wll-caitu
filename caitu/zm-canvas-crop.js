...
ZmCanvasCrop.prototype = {
    ...
    init: function(opt) {
        var self = this;
        self._input = opt.fileInput; //读取文件

        self._$box = $('.canvas-box'); //背景图片
        self.readFile(); //读取文件
    ...
    readFile: function() { //读取文件
        ...
        if (typeof FileReader === 'undefined') { //是否支持fileReader
        ...
        function readFile() {
            ...
            if (!/image\/\w+/.test(file.type)) { //是否是图片
                alert("文件必须为图片！");
                return false;
            }
    ...
    drawCavDown: function(src) { //选择要裁剪的图片
        var self = this;
        //清除上一次的
        self._$box.html('');
        self._save = {};
    ...
    addUpCanvas: function() { //开始裁剪图片前景框
